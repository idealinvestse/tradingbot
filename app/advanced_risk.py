...
class CorrelationAnalyzer:
    ...
    def recommend_diversification(
        self, current_positions: dict[str, float], available_strategies: list[str]
    ) -> list[str]:
        """
...
class PortfolioRiskManager:
    ...
    def calculate_portfolio_risk(
        self, positions: dict[str, dict], returns_history: pd.DataFrame
    ) -> PortfolioRisk:
        """
        ...
        if not returns_history.empty:
            ...
            sortino_ratio = (
                float(returns.mean() / downside_returns.std()) if len(downside_returns) > 0 else 0.0
            )
    ...
    def check_risk_limits(self, portfolio_risk: PortfolioRisk) -> tuple[bool, list[str]]:
        ...
        if portfolio_risk.correlation_risk > self.max_correlation:
            violations.append(
                f"Correlation risk {portfolio_risk.correlation_risk:.2f} exceeds limit {self.max_correlation}"
            )

        # Check concentration risk
        if portfolio_risk.concentration_risk > self.max_concentration:
            violations.append(
                f"Concentration risk {portfolio_risk.concentration_risk:.2f} exceeds limit {self.max_concentration}"
            )

        # Check max drawdown
        if abs(portfolio_risk.max_drawdown) > self.max_portfolio_risk:
            violations.append(
                f"Max drawdown {abs(portfolio_risk.max_drawdown):.2f} exceeds limit {self.max_portfolio_risk}"
            )
