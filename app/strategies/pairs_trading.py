...
class PairAnalyzer:
    ...
    def find_cointegrated_pairs(self, data: Dict[str, pd.DataFrame]) -> List[Tuple[str, str, float]]:
            ...
            for j in range(i + 1, len(symbols)):
                ...
                if pvalue < self.config.cointegration_pvalue:
                    ...
                    if correlation > self.config.min_correlation:
                        ...
                        logger.info(f"Found cointegrated pair: {symbol1}-{symbol2}, p-value: {pvalue:.4f}")
        ...
        for pair in self.pairs:
            ...
            if asset1 in market_data and asset2 in market_data:
                ...
                if pair_signals.iloc[-1] != 0:
                    ...
                    hedge_ratio = analyzer._calculate_hedge_ratio(
                        np.log(price1), np.log(price2)
                    )
