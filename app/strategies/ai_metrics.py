...
class AIMetricsTracker:
    ...
    def get_all_strategies_summary(self) -> List[Dict[str, Any]]:
        ...
        for strategy_name, metrics in self.metrics.items():
            summary = {
                ...
                "win_rate": f"{metrics.win_rate:.2%}",
                "total_return": f"{metrics.total_return:.2f}",
                "last_signal": metrics.last_signal_time.isoformat() if metrics.last_signal_time else "Never"
            }
    ...
    def take_performance_snapshot(self) -> None:
        ...
        snapshot = {
            "timestamp": datetime.utcnow(),
            "metrics": {
                name: metrics.dict() for name, metrics in self.metrics.items()
            },
    ...
    def get_performance_trend(
        self, strategy_name: str, lookback_days: int = 7
    ) -> Dict[str, Any]:
