...
class OHLCVValidator:
    ...
    def validate(self, df: pd.DataFrame) -> tuple[pd.DataFrame, DataQualityMetrics]:
        ...
        if "date" in clean_df.columns:
            ...
            expected_diff = (
                time_diff.mode()[0] if len(time_diff.mode()) > 0 else pd.Timedelta(minutes=5)
            )
        ...
        self.logger.info(
            "validation_complete",
            extra={"quality_score": quality_metrics.quality_score, "issues_fixed": total_issues},
        )

        return clean_df, quality_metrics

    def validate_live_tick(self, tick: dict) -> tuple[bool, str | None]:
        """
...
class DataCleaner:
    ...
    def normalize_volume(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "volume" in df.columns:
            window = 20
            df["volume_zscore"] = (df["volume"] - df["volume"].rolling(window).mean()) / df[
                "volume"
            ].rolling(window).std()
