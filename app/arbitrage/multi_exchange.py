...
class ArbitrageDetector:
    ...
    def find_opportunities(self, orderbooks: dict[str, dict]) -> list[ArbitrageOpportunity]:
        ...
        for symbol in all_symbols:
            ...
            exchanges_with_symbol = [
                exchange for exchange in orderbooks if symbol in orderbooks[exchange]
            ]
            ...
            for i, buy_exchange in enumerate(exchanges_with_symbol):
                for sell_exchange in exchanges_with_symbol[i + 1 :]:
                    opp = self._check_pair(symbol, buy_exchange, sell_exchange, orderbooks)
                    if opp:
                        opportunities.append(opp)

                    # Check reverse
                    opp_reverse = self._check_pair(symbol, sell_exchange, buy_exchange, orderbooks)
                    if opp_reverse:
    ...
    def _check_pair(
        self, symbol: str, buy_exchange: str, sell_exchange: str, orderbooks: dict
    ...
    def calculate_execution_risk(self, opportunity: ArbitrageOpportunity) -> float:
        ...
        # Profit margin risk
        margin_risk = max(
            0, 1 - (opportunity.profit_percentage / 1.0)
        )  # Risk increases as profit decreases
...
class ArbitrageExecutor:
    ...
    def execute_opportunity(self, opportunity: ArbitrageOpportunity) -> dict:
        ...
        try:
            ...
            buy_cost = buy_order.get("filled_amount", 0) * buy_order.get(
                "avg_price", opportunity.buy_price
            )
            sell_revenue = sell_order.get("filled_amount", 0) * sell_order.get(
                "avg_price", opportunity.sell_price
            )

            buy_fee = buy_cost * self.config.fee_structure.get(opportunity.buy_exchange, 0.002)
            sell_fee = sell_revenue * self.config.fee_structure.get(
                opportunity.sell_exchange, 0.002
            )
    ...
    def _place_order(
        self, exchange: str, symbol: str, side: str, amount: float, price: float
    ) -> dict:
...
class TriangularArbitrage:
        ...
        if self.base_currency in currencies:
            for curr1 in currencies:
                ...
                for curr2 in currencies:
                    ...
                    if (
                        f"{curr1}/{self.base_currency}" in symbols
                        and f"{curr2}/{curr1}" in symbols
                        and f"{self.base_currency}/{curr2}" in symbols
                    ):
    ...
    def _calculate_triangle_profit(
        self, triangle: tuple[str, str, str], exchange_data: dict
    ) -> float:
