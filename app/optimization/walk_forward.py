...
class WalkForwardOptimizer:
    ...
    def create_windows(self, data: pd.DataFrame) -> List[WalkForwardWindow]:
        ...
        while start_idx + self.config.in_sample_periods + self.config.out_sample_periods <= len(data):
    ...
    def optimize_window(self, window: WalkForwardWindow, data: pd.DataFrame,
                       strategy_class: Any) -> WalkForwardWindow:
        ...
        in_sample_metrics = self._evaluate_strategy(
            train_data, strategy_class, optimal_params
        )
        ...
        out_sample_metrics = self._evaluate_strategy(
            test_data, strategy_class, optimal_params
        )
        window.out_sample_performance = out_sample_metrics
        
        logger.info(f"Window {window.window_id}: IS {self.config.optimization_metric}="
    ...
    def _evaluate_strategy(self, data: pd.DataFrame, strategy_class: Any,
    ...
    def analyze_results(self) -> Dict:
        ...
        return {
            ...
            "oos_sharpe": np.mean(out_sample_scores) / np.std(out_sample_scores) if np.std(out_sample_scores) > 0 else 0
        }
