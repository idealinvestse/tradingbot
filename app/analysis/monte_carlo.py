...
class MonteCarloSimulator:
    ...
    def _parametric_simulation(self, returns: pd.Series) -> np.ndarray:
        ...
        simulated = np.random.normal(
            mu, sigma, size=(self.config.num_simulations, self.config.time_horizon)
        )
    ...
    def calculate_var(
        self, portfolio_value: float, returns: np.ndarray, confidence_level: float = 0.95
    ) -> float:
    ...
    def simulate_portfolio_paths(self, initial_value: float, returns: pd.Series) -> np.ndarray:
    ...
    def calculate_probability_metrics(self, paths: np.ndarray, initial_value: float) -> dict:
...
class RiskScenarioAnalyzer:
    ...
    def stress_test(
        self, portfolio_value: float, returns: pd.Series, scenarios: dict[str, float]
    ) -> dict:
    ...
    def tail_risk_analysis(self, returns: pd.Series) -> dict:
        ...
        return {
            ...
            "extreme_value_params": {"shape": shape, "location": loc, "scale": scale},
        }
...
class PortfolioOptimizer:
    ...
    def optimize_allocation(
        self, returns_dict: dict[str, pd.Series], target_return: float = 0.1, max_risk: float = 0.2
    ) -> dict[str, float]:
        ...
        if best_weights is not None:
            return {asset: weight for asset, weight in zip(assets, best_weights, strict=False)}
        else:
