...
class IntegrationTestStrategy(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe["fast_ma"] = dataframe["close"].rolling(window=10, min_periods=10).mean()
        dataframe["slow_ma"] = dataframe["close"].rolling(window=25, min_periods=25).mean()

        # Bullish crossover when fast crosses above slow on this candle
        crossover_signal = (dataframe["fast_ma"] > dataframe["slow_ma"]) & (
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if "enter_long" in dataframe.columns:
            dataframe.loc[(dataframe["enter_long"].shift(1) == 1), "exit_long"] = 1
