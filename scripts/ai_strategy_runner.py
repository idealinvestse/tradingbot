...
class AIStrategyRunner:
    ...
    async def run_single_strategy(self, strategy_name: str):
        ...
        strategy = next(
            (s for s in strategies if strategy_name.lower() in s.name.lower()),
            None
        )
        ...
        if strategy_name:
            ...
            if db_metrics:
                ...
                for metric in db_metrics:
                    print(f"  {metric['strategy_name']}: Return={metric['total_return']:.2f}, Win Rate={metric['win_rate']:.2%}")
...
async def main():
    ...
    parser.add_argument(
        "--strategy", "-s",
        help="Strategy name for single run"
    )
    parser.add_argument(
        "--file", "-f",
        help="File for export/import operations"
    )
    parser.add_argument(
        "--db", "-d",
        default="user_data/backtest_results/index.db",
        help="Database path"
    )
