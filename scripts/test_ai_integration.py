...
async def test_integration() -> bool:
    ...
    result = await run_ai_strategies(symbol="BTC/USDT", timeout=30)

    if result["success"]:
        print("[OK] All strategies executed successfully")
    ...
    result = await run_ai_strategies(
        symbol="ETH/USDT", strategy_type="predictive_modeling", timeout=30
    )

    if result["success"]:
        print("[OK] Predictive modeling strategy executed successfully")
        print(f"  - Total strategies: {result['total_strategies']}")
        print(f"  - Signals generated: {result['signals_generated']}")
    else:
        print(f"[FAIL] Execution failed: {result.get('error', 'Unknown error')}")
    ...
    result = await run_ai_strategies(symbol="BTC/USDT", strategy_type="arbitrage", timeout=30)

    if result["success"]:
        print("[OK] Arbitrage strategy executed successfully")
        print(f"  - Total strategies: {result['total_strategies']}")
        print(f"  - Signals generated: {result['signals_generated']}")
    else:
        print(f"[FAIL] Execution failed: {result.get('error', 'Unknown error')}")
