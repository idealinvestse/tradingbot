...
strategy.json
 ...
 - `docker-compose.yml` – lokal drift (Windows)
 ...
 - Profilering: Kör profiler vid större ändringar; dokumentera fynd.
 - Cache: OHLCV‑cache per market/timeframe med invalidation på ny data.
 - Parallellism: Multiprocess för backtests; undvik GIL‑hotspots.
 ...
 ## Containers och drift (Windows‑fokus)
 - Docker: Multi‑stage build, non‑root, healthcheck. Inga hemligheter i image.
 - Compose: Volymer för `user_data/` (data + configs). En tjänst per process.
 ...
 - Conventional Commits: `feat:`, `fix:`, `docs:`, `refactor:` m.fl.
 ...
 ## Dokumentation
 - README för snabbstart (testnet) + lokala körinstruktioner (Windows).
